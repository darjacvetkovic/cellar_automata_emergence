...
class TemporalDynamicsFrance(Dataset):
    def __init__(self, pkl_path, config, key='popagglo1780'):
        
             
        with open(pkl_path, 'rb') as f:
            all_grid_population = pickle.load(f)

        # put tmp into a tensor of size [1, 1, 20, 20]
        data_france = np.ndarray((len(all_grid_population), 1, len(all_grid_population[key]), len(all_grid_population[key])))
        for i, key in enumerate(all_grid_population):
            # normalize
            grid = all_grid_population[key]
            grid = np.log10(grid)
            grid[grid==-np.inf] = 0
            grid = grid/np.sum(grid)
            
            # scale grid values to [0, 1]
            grid = grid - np.min(grid)
            grid = grid / np.max(grid)

            data_france[i, 0, :, :] = grid

               
        dynamics_france = torch.tensor(data_france, dtype=torch.float32, requires_grad=True)
        self.initial_state = dynamics_france[0]
        self.target_dynamics = dynamics_france

        config["initial_state"] = self.initial_state
        config["target_dynamics"] = self.target_dynamics
    ...
    def __getitem__(self, index):
        return self.initial_state, self.target_dynamics
...
def train_dynamics(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            xx = torch.zeros((x.shape[0], x.shape[1]+extra_hidden_channels, x.shape[2], x.shape[3]))
            xx[:, :extra_hidden_channels, :, :] = x
            x = torch.tensor(xx, dtype=torch.float32, requires_grad=True)
            optimizer.zero_grad()
...
def run_nca_dynamics(model, initial_state, dynamic_steps, additive_update_delta, state_activation, clamp):
    ...
    for step in range(dynamic_steps):
        ...
        #next_state = next_state / torch.sum(next_state, dim=(2,3), keepdim=True) * 1000
